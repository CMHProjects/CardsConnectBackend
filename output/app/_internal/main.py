...
def process_single_sim_card(port, baud_rate, iccid_pin_data):
...
# Main function to process SIM cards
def process_sim_cards(port=None):
    baud_rate = 115200
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {executor.submit(process_single_sim_card, port, baud_rate, iccid_pin_data): port for port in active_ports}
...
if __name__ == "__main__":
    ...
    if args.port:
        process_sim_cards(port=args.port)
    else:
        process_sim_cards()
